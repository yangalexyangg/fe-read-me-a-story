<script lang="ts">
    ...
    const handleRegister = async () => {
        ...
        const userAndFamily = await createNewuserAndFamily(user.email, user.fullName, user.displayName, user.password, family.familyName)
        isNewUser = false
...
<p class="text-amber-100">Please fill in these details to create a new account</p>
    <form on:submit = {handleRegister}>
        ...
        <input bind:value = {user.fullName} type = "text" class="mb-6 rounded bg-amber-100 p-2" required><br>
        <label for="nickname">Your role</label><br>
        <input bind:value = {user.displayName} type="text" placeholder = "Grampy" class="mb-6 rounded bg-amber-100 p-2" required><br>
        <label for="familyName">Your family's name</label><br>
        <input bind:value = {family.familyName} type="text" placeholder = "Hamilton Family" class="mb-6 rounded bg-amber-100 p-2" required><br>
